...
async function fetchAllSolvedProblems(handle) {
    ...
    while (true) {
        ...
        if (!res.ok) {
            throw new Error(
                `search/problem failed for ${handle} page=${page}: ${res.status}`,
            );
        }
...
async function main() {
    ...
    const handles = (
        Array.isArray(rivals.handles) ? rivals.handles : []
    ...
    for (const handle of handles) {
        const previous = nextState[handle] || {
            solvedCount: 0,
            seenProblemIds: [],
        };
        ...
        try {
            ...
            const seenProblemIds = Array.isArray(previous.seenProblemIds)
                ? previous.seenProblemIds.map(String)
                : [];
            ...
            if (!hasCompleteHistory) {
                ...
                console.log(
                    `Backfilled solved problems for ${handle}: ${mergedSeenProblemIds.length}`,
                );
            } else if (solvedCount > previousSolvedCount) {
                const allSolvedProblems = await fetchAllSolvedProblems(handle);
                newlySolved = allSolvedProblems.filter(
                    (p) => !seenSet.has(p.id),
                );
                mergedSeenProblemIds = mergeSeenProblemIds(
                    seenProblemIds,
                    allSolvedProblems.map((p) => p.id),
                );
            }
    ...
    if (webhook) {
        for (const n of notifications) {
            ...
            try {
                ...
                console.error(
                    `Slack send failed for ${n.handle}: ${error.message}`,
                );
            }
        }
    } else {
